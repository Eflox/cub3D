...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/01 18:28:39 by chde-mar          #+#    #+#             */
/*   Updated: 2021/04/06 00:51:07 by chde-mar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	start_game(t_index *m, char *av)
{
	create_init(m);
	if (parse_cub(m, av) < 0)
		return (-1);
	m->win.mlx_ptr = mlx_init();
	if (!m->win.mlx_ptr)
		return (-1);
...
int	main(int ac, char **av)
{
	...
	{
		if ((start_game(&m, av[1])) < 0)
			return (exit_all(&m));
		mlx_hook(m.win.mlx_win, 2, 1L << 0, ft_key_states, &m);
		mlx_hook(m.win.mlx_win, 3, 1L << 1, ft_key_off, &m);
