...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/01 18:29:18 by chde-mar          #+#    #+#             */
/*   Updated: 2021/04/06 01:04:43 by chde-mar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	m->tex.color_e = mlx_xpm_file_to_image(m->win.mlx_ptr, m->el.e_path,
	...
	m->spr.spr_tex = mlx_xpm_file_to_image(m->win.mlx_ptr, m->el.spr_path,
...
{
	m->tex.color_n = mlx_xpm_file_to_image(m->win.mlx_ptr, m->el.n_path,
	...
	m->tex.color_s = mlx_xpm_file_to_image(m->win.mlx_ptr, m->el.s_path,
			&m->tex.texwidth, &m->tex.texheight);
	if (!m->tex.color_s)
	{
		write(1, "Error\n", 6);
	...
	m->tex.color_w = mlx_xpm_file_to_image(m->win.mlx_ptr, m->el.w_path,
			&m->tex.texwidth, &m->tex.texheight);
	if (!m->tex.color_w)
	{
		write(1, "Error\n", 6);
